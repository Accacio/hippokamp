...
- if $dim(e)<dim(v)$: reduced rank
  ...
  - Spectral density $\Phi_v(z)=H(z)\LambdaH^T(z^{-1})$ is singular
*** Disturbance modeling - reduced rank noise
...
** Network Identifiability
- under which conditions does $\Phi_{\bar v}=(I-G)^{-1}H\LambdaH^\star(I-G)^{-\star}$ provides a unique $T_{we}=(I-G)^-1H$
- Proposition
  1. modules in G are strictly proper
  2. no alebraic loops in G(\theta) and $H^\infty\Lmabda {H^\infty}^T$
